...
corr_inds <- vector("list", length = simnum)
corr_exchs <- vector("list", length = simnum)
corr_ar1s <- vector("list", length = simnum)
...
for (s in 1:simnum) {
  ...
  survtimes <- simsurv(dist = "exponential", lambdas = 0, 
                       betas = c(agei = 0.01, bavi = -0.5), x = covs, maxt = 4)
  
  
  # generate outcome values
  bi <- rnorm(N, mean = 0, sd = 5) #bi(sd=0/1/5)----
  b <- rep(bi, each = maxT)
  ...
  # * b) sample data----
  #1) independece
  gee_inds <- geeglm(root ~  visit + bav + bav:visit, data = matched_long, family = gaussian,
                     id = id, waves = visit, corstr = "independence")
  ...
  outvec_inds[[s]] <- c(s, K, unlist(estbeta_inds[[s]]), unlist(se_inds[[s]]), unlist(cse_inds[[s]]),
                        unlist(relbias_inds[[s]]), unlist(mse_inds[[s]]), unlist(cp_inds[[s]]), 
                        unlist(ccp_inds[[s]]),unlist(corr_inds[[s]]))
  
  #2) exchangeable
  gee_exchs <- geeglm(root ~  visit+ bav + bav:visit, data = matched_long, family = gaussian,
                      id = id, waves = visit, corstr = "exchangeable")
  ...
  corr_exchs[[s]] <- summary(gee_exchs)$corr[1]
  outvec_exchs[[s]] <- c(s, K, unlist(estbeta_exchs[[s]]), unlist(se_exchs[[s]]), unlist(cse_exchs[[s]]),
                         unlist(relbias_exchs[[s]]), unlist(mse_exchs[[s]]), unlist(cp_exchs[[s]]), 
                         unlist(ccp_exchs[[s]]),unlist(corr_exchs[[s]]))
  #3) AR1
  gee_ar1s <- geeglm(root ~  visit+ bav + bav:visit, data = matched_long, family = gaussian,
                     id = id, waves = visit, corstr = "ar1")
  ...
  corr_ar1s[[s]] <- summary(gee_ar1s)$corr[1]
  outvec_ar1s[[s]] <- c(s, K, unlist(estbeta_ar1s[[s]]), unlist(se_ar1s[[s]]), unlist(cse_ar1s[[s]]),
                        unlist(relbias_ar1s[[s]]), unlist(mse_ar1s[[s]]), unlist(cp_ar1s[[s]]), 
                        unlist(ccp_ar1s[[s]]),unlist(corr_ar1s[[s]]))
  
  #4) Unstructur
  # gee_unstrs <- geeglm(root ~  visit+ bav + bav:visit, data = matched_long, family = gaussian,
  #                      id = id, waves = visit, corstr = "unstructured")
  ...
  # lme <- lmer(root ~  visit+ bav + bav:visit + (1|id), data = matched_long)
  # estbeta_lme[[s]] <- summary(lme)$coef[,1]
  # se_lme[[s]] <- summary(lme)$coef[,2]
  # ci_lme <- confint(lme)[3:6,]
  # for (i in 1:length(beta_true_s)) {
  ...
  # }
  # outvec_lme[[s]] <- c(s,K, unlist(estbeta_lme[[s]]),unlist(se_lme[[s]]),NA,NA,NA,NA,
  #                      unlist(relbias_lme[[s]]),unlist(mse_lme[[s]]), unlist(cp_lme[[s]]),NA,NA,NA,NA)
...
names1 <- c("simnum", "sample_size", "(intercept)", "visit", "bav","vist:bav",
...
#                   sd(outvec_inds[,3]), sd(outvec_inds[,4]),sd(outvec_inds[,5]),sd(outvec_inds[,6]))
# outmean_exchs <- c(simnum,median(outvec_exchs[,2]),colMeans(outvec_exchs[,c(-1,-2)]),sd(outvec_exchs[,3]),
#                    sd(outvec_exchs[,4]),sd(outvec_exchs[,5]),sd(outvec_exchs[,6]))
# outmean_ar1s <- c(simnum,median(outvec_ar1s[,2]),colMeans(outvec_ar1s[,c(-1,-2)]),sd(outvec_ar1s[,3]),
#                   sd(outvec_ar1s[,4]),sd(outvec_ar1s[,5]),sd(outvec_ar1s[,6]))


# outmean_unstrs <- c(simnum,median(outvec_unstrs[,2]),colMeans(outvec_unstrs[,c(-1,-2)]),sd(outvec_unstrs[,3]),
#                     sd(outvec_unstrs[,4]),sd(outvec_unstrs[,5]),sd(outvec_unstrs[,6]))
# outmean_lme <- c(simnum,median(outvec_lme[,2]), colMeans(outvec_lme[,c(-1,-2)]), sd(outvec_lme[,3]),
#                  sd(outvec_lme[,4]),sd(outvec_lme[,5]),sd(outvec_lme[,6]))

# Model <- c("Independence","","","","Exchangeable","","","", "AR(1)","","","","Unstructured","","","",
#            "Linear mixed effect","","","")
# Parameters <- rep(c("$\\beta_0$(int)","$\\beta_1$(Time)","$\\beta_2$(BAV)","$\\beta_3$(BAV*Time)"),5)
# SampleSize1 <- rep(outmean_inds[2],20)
# MeanEstimates <- c(outmean_inds[3:6], outmean_exchs[3:6],outmean_ar1s[3:6],outmean_unstrs[3:6],outmean_lme[3:6])
# TrueValues <- rep(beta_true_s,5)
# MeanSE <- c(outmean_inds[7:10], outmean_exchs[7:10],outmean_ar1s[7:10],outmean_unstrs[7:10], outmean_lme[7:10])
# MeanSE_adj <- c(outmean_inds[11:14], outmean_exchs[11:14],outmean_ar1s[11:14],outmean_unstrs[11:14],outmean_lme[11:14])
# SD <- c(outmean_inds[31:34], outmean_exchs[31:34],outmean_ar1s[31:34],outmean_unstrs[31:34],outmean_lme[31:34])
# MeanRelBias <- c(outmean_inds[15:18], outmean_exchs[15:18],outmean_ar1s[15:18],outmean_unstrs[15:18],outmean_lme[15:18])
# MSE <- c(outmean_inds[19:22], outmean_exchs[19:22],outmean_ar1s[19:22],outmean_unstrs[19:22],outmean_lme[19:22])
# CovProb <- c(outmean_inds[23:26], outmean_exchs[23:26],outmean_ar1s[23:26],outmean_unstrs[23:26],outmean_lme[23:26])
# AdjCovProb <- c(outmean_inds[27:30], outmean_exchs[27:30],outmean_ar1s[27:30],outmean_unstrs[27:30],outmean_lme[27:30])
# 
...
# 
# out_t1 <- kableExtra::kable(t2, row.names=FALSE, escape = FALSE,
...
#   kable_styling(full_width = F, position = "center") %>%
#   #collapse_rows(columns = 1, valign = "middle") %>%
#   row_spec(c(4,8,12,16,20), background = "lightgrey") %>%
#   column_spec(1:2, background = "transparent") 
# 
# result1 <- t2[1:16,c(-2,-6,-11)]
# out_t2 <- kableExtra::kable(result1, row.names=FALSE, escape = FALSE,
#                             caption = "GEE models comparison for matched sample with 1000 simulations",
#                             col.names = c("Model","Parameter","True value","Mean Est",
#                                           "Mean SE","Emp SE","Mean Rel Bias","MSE","Cov prob")) %>% 
#   kable_styling(full_width = F, position = "center") %>%
#   row_spec(c(4,8,12,16), background = "lightgrey") %>%
#   column_spec(1:2, background = "transparent") 