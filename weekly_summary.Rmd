...
1. In the case study Arthritis Clinical Trial in Chapter 13:
  * Why the model used square-root transformation for time(month)?
  * Why the GEE estimates in Table 13.12 is different from the results in the sample R program?
  * Why do the estimates become "NaN" when changing the independent working correlation to an unstructured working correlation?
  
2. Dichotomous response variable:
* Why the sample code uses "ordgee()"function with logit link to fit Marginal Logistic Regression Model?
* geeglm() gives same result as ordgee() with logit link.
