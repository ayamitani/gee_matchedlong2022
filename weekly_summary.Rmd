...
1. Since I dropped rows that contained missing values after converting the data into long format, does it matter that some subjects do not have all the 4 measurements (and we specified "waves=time")? 

2. When should we set scale.fix to be false? Why the scale parameter becomes 1.01 instead of 1 when only included 20 subjects?

3. Why there's only 1 estimate (alpha) for the correlation parameters using "ar1" correlation structure?

4. QIC() function provides not only QIC values, do we also need other criterions for the comparison tables?

5. How to interpret the results based on the comparisons?
...
6. For propensity score matching, do we need to consider caliper distance when performing the matching?
